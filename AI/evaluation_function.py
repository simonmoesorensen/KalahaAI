def evaluation_function(game):
    ...
    # - Extra turn
    # - Avoid steal
    # - Avoid opponent extra turn
    # - Score on each side = 1 pr piece (y)

    utility += check_player_utility(opposite_player_turn, state, utility)
    utility += check_player_utility(player_turn, state, utility)

    # Mancala score utility

    # Compute player 1 score (max)
    player_1_score = state[0][-1]

    # Compute player 2 score (min)
    player_2_score = state[1][-1]

    utility += player_1_score - player_2_score
    return utility


def check_player_utility(player_turn, state, utility):
    opposite_player_turn = 1 if player_turn == 0 else 0
    # Check steal and extra turn
    for slot, slot_count in enumerate(state[player_turn][0:6]):
        # Extra turn
        if (6 - slot) == slot_count:
            utility += -1 if player_turn == 1 else 1
        ...
        if slot_count == 0 and pieces_to_steal > 0:
            ...
            for slot2, slot_count2 in enumerate(state[player_turn][0:slot]):
                if (slot - slot2) == slot_count2:
                    ...
                    utility += -pieces_to_steal - 1 if player_turn == 1 else pieces_to_steal + 1
